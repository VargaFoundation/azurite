...
adls_secure_mode = config['configurations'].get('azure-storage-site', {}).get('azure.adls.secure.mode', 'false')
storage_endpoint_suffix = config['configurations'].get('azure-storage-site', {}).get('azure.storage.endpoint.suffix', 'core.windows.net')
...
if azure_storage_backend == 'adls_gen2':
    adls_fqdn = '{0}.dfs.{1}'.format(storage_account_name, storage_endpoint_suffix)
    adls_scheme = 'abfss' if adls_secure_mode == 'true' else 'abfs'
    fs_default_fs = '{0}://{1}@{2}/'.format(adls_scheme, storage_container_name, adls_fqdn)
elif azure_storage_backend == 'wasb':
    wasb_fqdn = '{0}.blob.{1}'.format(storage_account_name, storage_endpoint_suffix)
